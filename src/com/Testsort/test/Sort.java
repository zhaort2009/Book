...
其中<AnyType extends Comparable<? super AnyType>>是泛型的类型，定义类型只能是Comparable或Comparable的子类，
并且Comparable也是一个泛型类他的 类型只能是AnyType或是AayType的超类也就是AayType父类
int 是方法的返回类型(AnyType[] a,AnyType x)参数列表，
第一个是传递进来的数组第二个是一个值，也就是查找该值在数组中的下标，他们的类型都是该泛型方发传进来的类型*/
